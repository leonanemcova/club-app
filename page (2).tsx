      ...
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        :root{
          ...
          --blue:#0038FF;--muted:#8A8880;--border:rgba(10,10,15,.10);
        }
        body{background:var(--paper);color:var(--ink);font-family:'DM Sans',sans-serif;overflow-x:hidden}
        body::before{
          content:'';position:fixed;inset:0;pointer-events:none;z-index:99;opacity:.35;
        ...
        .a1{animation:fadeUp .65s .00s both}.a2{animation:fadeUp .65s .08s both}
        .a3{animation:fadeUp .65s .16s both}.a4{animation:fadeUp .65s .24s both}
        .a5{animation:fadeUp .65s .32s both}

        /* Nav */
        .nav{position:sticky;top:0;z-index:50;display:flex;align-items:center;justify-content:space-between;
          padding:18px 40px;background:rgba(245,243,238,.88);backdrop-filter:blur(14px);
          border-bottom:1px solid var(--border)}
        .logo{display:flex;align-items:center;gap:10px;text-decoration:none}
        .logo-mark{width:32px;height:32px;border-radius:8px;background:var(--ink);display:flex;
          align-items:center;justify-content:center;font-family:'Syne',sans-serif;
          font-size:9px;font-weight:800;color:#FFD600;letter-spacing:.04em;flex-shrink:0}
        .logo-name{font-family:'Syne',sans-serif;font-weight:800;font-size:13px;
          letter-spacing:.1em;text-transform:uppercase;color:var(--ink)}
        .nav-links{display:flex;align-items:center;gap:6px}
        .btn-ghost{padding:8px 18px;font-size:13px;font-weight:500;color:var(--muted);
          background:none;border:none;border-radius:8px;cursor:pointer;text-decoration:none;
          transition:color .15s}
        .btn-ghost:hover{color:var(--ink)}
        .btn-solid{padding:9px 20px;font-size:13px;font-weight:600;color:var(--paper);
          background:var(--ink);border:none;border-radius:8px;cursor:pointer;text-decoration:none;
          transition:opacity .15s,transform .1s}
        .btn-solid:hover{opacity:.85;transform:translateY(-1px)}

        /* Hero */
        .hero{max-width:1100px;margin:0 auto;padding:72px 40px 56px;
        ...
        .h1{font-family:'Syne',sans-serif;font-weight:800;font-size:clamp(42px,5.5vw,74px);
          line-height:1.0;letter-spacing:-.03em;color:var(--ink);margin-bottom:22px}
        .h1 em{font-style:normal;color:var(--accent)}
        .lead{font-size:16px;line-height:1.65;color:var(--muted);font-weight:300;
          max-width:450px;margin-bottom:36px}
        .cta-row{display:flex;gap:12px;flex-wrap:wrap}
        .btn-primary{padding:13px 30px;font-size:14px;font-weight:600;color:#fff;
          background:var(--accent);border:none;border-radius:10px;cursor:pointer;
          text-decoration:none;display:inline-flex;align-items:center;gap:8px;
          box-shadow:0 4px 20px rgba(255,77,28,.28);transition:all .15s}
        .btn-primary:hover{transform:translateY(-2px);box-shadow:0 8px 28px rgba(255,77,28,.38)}
        .btn-outline{padding:13px 30px;font-size:14px;font-weight:500;color:var(--ink);
        ...
        /* Card */
        .card{background:var(--ink);border-radius:20px;padding:26px;
          animation:floatCard 6s ease-in-out infinite;position:relative;overflow:hidden}
        .card::before{content:'';position:absolute;top:-40px;right:-30px;width:180px;height:180px;
          background:var(--blue);border-radius:50%;opacity:.10;filter:blur(40px)}
        .card-label{font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;
          color:rgba(255,255,255,.28);margin-bottom:14px}
        .match-row{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.07);
          border-radius:11px;padding:12px 14px;margin-bottom:7px;
          display:flex;align-items:center;gap:10px}
        .m-tag{font-size:9px;font-weight:700;letter-spacing:.07em;text-transform:uppercase;
        ...
        .m-teams{font-family:'Syne',sans-serif;font-size:11px;font-weight:700;color:#fff;flex:1}
        ...
        .card-stats{margin-top:18px;display:grid;grid-template-columns:repeat(3,1fr);gap:7px}
        .stat{background:rgba(255,255,255,.05);border-radius:10px;padding:11px 8px;text-align:center}
        .stat-n{font-family:'Syne',sans-serif;font-size:20px;font-weight:800;color:#fff;
          line-height:1;margin-bottom:3px}
        .stat-l{font-size:8px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;
          color:rgba(255,255,255,.25)}

        /* Ticker */
        .ticker{border-top:1px solid var(--border);border-bottom:1px solid var(--border);
          padding:11px 0;overflow:hidden;white-space:nowrap;background:rgba(10,10,15,.025)}
        .ticker-track{display:inline-flex;animation:marquee 28s linear infinite}
        .t-item{display:inline-flex;align-items:center;gap:8px;padding:0 28px;font-size:11px;
        ...
        .sec-tag{font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;
          color:var(--accent);margin-bottom:8px}
        .sec-h2{font-family:'Syne',sans-serif;font-size:clamp(26px,3.5vw,38px);font-weight:800;
          letter-spacing:-.02em;color:var(--ink)}
        .feat-num{font-family:'Syne',sans-serif;font-size:60px;font-weight:800;
          color:rgba(10,10,15,.05);line-height:1}
        .feat-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}
        .feat-card{padding:26px;border:1.5px solid var(--border);border-radius:14px;
          background:rgba(255,255,255,.5);transition:all .2s}
        .feat-card:hover{transform:translateY(-3px);box-shadow:0 10px 36px rgba(10,10,15,.08);
          background:#fff;border-color:rgba(10,10,15,.18)}
        .feat-wide{grid-column:span 2;background:var(--ink) !important;border-color:var(--ink) !important;
          display:flex !important;align-items:flex-start;gap:28px}
        .feat-wide:hover{transform:translateY(-3px);box-shadow:0 12px 40px rgba(10,10,15,.25) !important}
        .feat-icon{width:42px;height:42px;background:rgba(10,10,15,.06);border-radius:10px;
          display:flex;align-items:center;justify-content:center;font-size:20px;
          margin-bottom:18px;flex-shrink:0}
        .feat-wide .feat-icon{background:rgba(255,255,255,.08);margin-bottom:0}
        .feat-title{font-family:'Syne',sans-serif;font-size:15px;font-weight:700;
          color:var(--ink);margin-bottom:7px}
        .feat-wide .feat-title{color:#fff !important;font-size:20px}
        .feat-desc{font-size:13px;line-height:1.6;color:var(--muted);font-weight:300}
        .feat-wide .feat-desc{color:rgba(255,255,255,.45) !important;font-size:13.5px}

        /* CTA */
        .cta-wrap{max-width:1100px;margin:0 auto 72px;padding:0 40px}
        .cta-box{background:var(--ink);border-radius:22px;padding:56px 60px;
          display:flex;align-items:center;justify-content:space-between;
          gap:40px;position:relative;overflow:hidden}
        .cta-box::before{content:'';position:absolute;top:-80px;right:80px;width:350px;height:350px;
          background:var(--blue);border-radius:50%;opacity:.07;filter:blur(70px)}
        .cta-box::after{content:'';position:absolute;bottom:-70px;right:-30px;width:280px;height:280px;
          background:var(--accent);border-radius:50%;opacity:.07;filter:blur(60px)}
        .cta-h{font-family:'Syne',sans-serif;font-size:clamp(22px,2.8vw,34px);font-weight:800;
        ...
        .btn-cta1{padding:13px 26px;font-size:13px;font-weight:700;color:var(--ink);
          background:var(--paper);border:none;border-radius:9px;cursor:pointer;
        ...
        .btn-cta2{padding:13px 26px;font-size:13px;font-weight:500;
        ...
        /* Footer */
        .footer{border-top:1px solid var(--border);padding:22px 40px;display:flex;
        ...
        @media(max-width:900px){
          ...
          .feat-wide{grid-column:span 2;flex-direction:column;gap:0}
          .feat-wide .feat-icon{margin-bottom:18px}
